...
dt_sum0 <- mar10dt %>% group_by(temp, rep, gate, ab, controlrep) %>%
...
ctrlcounts <- ctrlcounts %>% 
  group_by(temp, rep, ab, controlrep) %>% 
...
dt_adjCountsYFP <- dt_sum2 %>% 
  mutate(adjCount=dt_sum2$count/dt_sum2$proportion)
...
total <- merge(dt_adjCountsCU,dt_adjCountsYFP,by=c("temp","rep", "ab")) %>% 
  ...
  rename("adjCountYFP" = "adjCount", "adjCountCU" = "CU") %>% 
